...
void iterateDiff(FILE *p_fp, FILE *ip_fp, FILE *like_fp,
  ...
  fprintf(OUT, "\niterateDiff(): Acceptance ratio %.2f %%\n", 
...
void iterateDoubleDiff(FILE *p_fp, FILE *like_fp,
  ...
  fprintf(OUT, "\niterateDoubleDiff(): Acceptance ratio %.2f %%\n", 
	  (float)nAccepted/nIter*100);
  gsl_rng_free(r);
}
...
void iterateDouble(FILE *p_fp, FILE *like_fp,
  ...
  fprintf(OUT, "\niterateDouble(): Acceptance ratio %.2f %%\n", 
	  (float)nAccepted/nIter*100);
  gsl_rng_free(r);
}
...
void iterateRJ(char *k_prefix, 
  ...
  char * likeFmt="%s_likelihood%03i.dat";
